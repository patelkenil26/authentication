...
- **User Registration**: Secure email and password registration.
- **User Login**: Generates JWT Access Token (HS256) and Refresh Token.
- **Refresh Token Management**: Refresh tokens are stored securely in `httpOnly` cookies.
- **Account Verification**: Email verification flow using token links.
- **Password Management**: Forgot Password and Reset Password flows.
- **Protected Routes**: Middleware (`auth.middleware.js`) to protect routes using Bearer tokens.
...
- **Provider Architecture**: Implemented a highly scalable Strategy Pattern for third-party logins. Adding a new provider (e.g., GitHub, Facebook) only requires creating a single provider file without modifying routes or controllers.
